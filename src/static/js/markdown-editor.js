  ...
  const getRenderer = (() => {
    ...
    return () => {
      ...
      renderer.codespan = (code) =>
        `<code class="bg-[#1f2328] text-[var(--color-link)] text-xs rounded px-1 py-0.5">${escapeHtml(code)}</code>`;
      ...
      renderer.link = (href, title, text) => {
        ...
        return `<a class="text-[var(--color-link)] hover:text-indigo-200 underline" href="${escapeHtml(safeHref)}" target="_blank" rel="noopener noreferrer"${titleAttr}>${text}</a>`;
      };
